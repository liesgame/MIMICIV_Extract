  ...
  select
    ...
    , case
        when OxygenTherapy=1 then
          LAG(CHARTTIME, 1) OVER (partition by stay_id, OxygenTherapy order by charttime)
        else null
...
(
  select
      ...
      -- if the interval of charttime and last 1 charttime > 8, we think it is a next ventnum 
