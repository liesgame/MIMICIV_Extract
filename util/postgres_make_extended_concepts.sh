...
export MIMIC_CODE_DIR='D:/数据/香港实习/MIMICIV/mimic-code-main/mimic-iii'
...
# { echo "${PSQL_PREAMBLE}; DROP TABLE IF EXISTS colloid_bolus; CREATE TABLE colloid_bolus AS "; cat ./colloid_bolus.sql; } | sed -r -e "${REGEX_DATETIME_DIFF}" | sed -r -e "${REGEX_SCHEMA}"
